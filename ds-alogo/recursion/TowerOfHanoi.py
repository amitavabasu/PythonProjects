...
def tower_of_hanoi(n):
    ...
    """
    result = []
    move_disks_rec(n, 1, 3, 2, result)
    return result


def move_disks_rec(n, source, dest, aux, result):
    if n == 1:
        result.append([source, dest])
        return
    else:
        move_disks_rec(n - 1, source, aux, dest, result)
        result.append([source, dest])
        move_disks_rec(n - 1, aux, dest, source, result)
        return
